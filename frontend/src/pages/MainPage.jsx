...
function MainPage({ lang }) {
  ...
  const AllergyToggleBtn = () => (
    <button onClick={() => setShowAllergy(!showAllergy)} className={`text-[10px] md:text-xs font-black px-3 py-1.5 rounded-xl transition-all border shadow-sm flex items-center gap-1 ${showAllergy ? 'bg-orange-50 border-orange-200 text-orange-600 dark:bg-orange-900/30 dark:border-orange-800 dark:text-orange-400' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-600'}`}>{showAllergy ? '🚨 알레르기 켜짐' : '💡 알레르기 보기'}</button>
  );
  ...
  const renderFoodCard = (campusFilter) => (
    <div className="flex flex-col gap-3 pb-6">
      {!meals ? ( <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl text-center text-xs text-gray-400 font-bold animate-pulse">데이터 로딩중...</div> ) : meals.filter(m => m.place.includes(campusFilter)).length === 0 ? ( <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl text-center text-xs text-gray-400 font-bold">식단 정보가 없습니다.</div> ) : (
          ...
          return (
            <div key={idx} className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden">
              ...
              <ul className="pl-3 space-y-1 mt-2">
                {meal.menu.includes("운영중지") ? ( <li className="text-red-500 text-[11px] font-bold py-1">⚠️ 운영중지</li> ) : (
                  lines.map((line, i) => {
    ...
    <div className="min-h-screen flex flex-col transition-colors relative bg-gray-50/30 dark:bg-gray-900 overflow-x-hidden font-sans">
      ...
      <div id="tour-side-btns" className="fixed left-0 top-1/2 -translate-y-1/2 z-[100] flex flex-col gap-3">
        <button onClick={() => setIsBongrimOpen(true)} className="bg-blue-600 text-white pl-3 pr-4 py-4 rounded-r-2xl shadow-lg hover:bg-blue-700 transition-all flex flex-col items-center gap-2 group border border-l-0 border-blue-400">
          ...
          <span className="text-xs font-black tracking-widest" style={{ writingMode: 'vertical-rl' }}>봉림관 학식</span>
        </button>
        <button onClick={() => setIsSarimOpen(true)} className="bg-indigo-600 text-white pl-3 pr-4 py-4 rounded-r-2xl shadow-lg hover:bg-indigo-700 transition-all flex flex-col items-center gap-2 group border border-l-0 border-indigo-400">
          <span className="text-2xl group-hover:scale-110">🍱</span>
          <span className="text-xs font-black tracking-widest" style={{ writingMode: 'vertical-rl' }}>사림관 학식</span>
        </button>
      </div>
      ...
      <div className={`fixed top-0 left-0 h-full w-[300px] md:w-[350px] bg-gray-50 dark:bg-gray-900 shadow-2xl z-[200] transform transition-transform duration-300 flex flex-col ${isBongrimOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex flex-col gap-4 shadow-sm z-10">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl">
            <button onClick={() => setBongrimTab('1층')} className={`flex-1 py-1.5 rounded-lg text-xs font-black transition-all ${bongrimTab === '1층' ? 'bg-white dark:bg-gray-600 text-blue-600' : 'text-gray-500'}`}>1층 (MOSS1)</button>
            <button onClick={() => setBongrimTab('2층')} className={`flex-1 py-1.5 rounded-lg text-xs font-black transition-all ${bongrimTab === '2층' ? 'bg-white dark:bg-gray-600 text-blue-600' : 'text-gray-500'}`}>2층 (MOSS2)</button>
          </div>
        </div>
      ...
      <div className="relative max-w-7xl mx-auto w-full px-5 md:px-10 flex-grow flex flex-col justify-center mt-4 md:mt-0">
        ...
        <div className="flex justify-center mb-6 md:mb-8 pt-4">
          ...
          ) : ( <div className="inline-flex items-center gap-2 bg-gray-50 dark:bg-gray-800 px-5 py-3 rounded-full text-xs font-bold text-gray-400 animate-pulse mt-4">⏳ 실시간 정보 로딩...</div> )}
        </div>
        ...
        <div className="bg-blue-50/50 dark:bg-blue-900/20 p-8 md:p-12 rounded-[3.5rem] border-2 border-blue-100/50 dark:border-blue-800/50 relative overflow-hidden">
          <div className="text-center mb-16 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <a href="https://www.changwon.ac.kr/portal/na/ntt/selectNttList.do?mi=13532&bbsId=2932" target="_blank" rel="noreferrer" className="bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 font-black text-base px-8 py-4 rounded-3xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all border border-blue-100 dark:border-gray-700 text-center">{current.noticeBtn1} ↗</a>
              <a href="https://www.changwon.ac.kr/portal/main.do#" target="_blank" rel="noreferrer" className="bg-[#002f6c] dark:bg-blue-800 text-white font-black text-base px-8 py-4 rounded-3xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all border border-blue-900 dark:border-gray-700 text-center">{current.noticeBtn2} ↗</a>
            </div>
          </div>
      ...
      {/* 푸터 */}
      <footer className="py-8 md:py-12 text-center border-t border-gray-200 dark:border-gray-800 mt-16 relative z-10">
        <p className="text-gray-600 dark:text-gray-400 font-black text-[10px] md:text-sm uppercase tracking-widest mb-2">{current.footerDept}</p>
        <div className="flex items-center justify-center gap-4 mt-3">
          ...
          <a href="https://github.com/eryang11188/todo-app-mini-project-20222017.git" target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 transition-all hover:scale-110">
            <svg height="35" width="35" viewBox="0 0 16 16" fill="currentColor"><path d="M8 0c4.42 0 8 3.58 8 8a8.01 8.01 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" /></svg>
          </a>
        </div>
      </footer>
    </div>
